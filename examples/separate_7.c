...
int main(int argc, char* argv[])
{
	...
	FILE *fp_mic1, *fp_mic2, *fp_mic3, *fp_mic4, *fp_mic5, *fp_mic6, *fp_mic7, *fp_ref;

	WAVEHEADFMT waveheadmic1, waveheadmic2, waveheadmic3, waveheadmic4, waveheadmic5, waveheadmic6, waveheadmic7, waveheadref;
	...
	fp_mic5 = fopen("mic5.wav", "wb");
	if(fp_mic5 == NULL)
	{
		printf("Can't open mic5.wav file!\n");
	}
	fp_mic6 = fopen("mic6.wav", "wb");
	if(fp_mic6 == NULL)
	{
		printf("Can't open mic6.wav file!\n");
	}
	fp_mic7 = fopen("mic7.wav", "wb");
	if(fp_mic7 == NULL)
	{
		printf("Can't open mic7.wav file!\n");
	}
	fp_ref = fopen("ref.wav", "wb");
	if(fp_ref == NULL)
	{
		printf("Can't open ref.wav file!\n");
	}
		...
		for (i = 0; i < TOTAL_SAMPLE_NUM; i++) {
			...
			if (flag_pos == -1 && (data10ms[i] >= flag_s16[0]-5 && data10ms[i] <= flag_s16[0]+5)) {
				...
				if (data_index == frame_size) {
					...
					mic1[cntallper] = frame_buf[2];
					mic2[cntallper] = frame_buf[3];
					mic3[cntallper] = frame_buf[4];
					mic4[cntallper] = frame_buf[5];
					mic5[cntallper] = frame_buf[6];
					mic6[cntallper] = frame_buf[7];
					mic7[cntallper] = frame_buf[8];
					ref[cntallper] = frame_buf[9];
					cntallper++;
					if(cntallper >= 160)
					{
						cntallper = 0;
						...
						fwrite(mic5, sizeof(short), 160, fp_mic5);
						fwrite(mic6, sizeof(short), 160, fp_mic6);
						fwrite(mic7, sizeof(short), 160, fp_mic7);
						fwrite(ref, sizeof(short), 160, fp_ref);
					}
	...
	PrepareWaveHead( &waveheadmic5, 
		1,
		MIC_RATE,
		16,
		totalSampleCountper * sizeof(unsigned short)
		);
	WriteWaveHead( &waveheadmic5, fp_mic5);

	PrepareWaveHead( &waveheadmic6, 
		1,
		MIC_RATE,
		16,
		totalSampleCountper * sizeof(unsigned short)
		);
	WriteWaveHead( &waveheadmic6, fp_mic6);

	PrepareWaveHead( &waveheadmic7, 
		1,
		MIC_RATE,
		16,
		totalSampleCountper * sizeof(unsigned short)
		);
	WriteWaveHead( &waveheadmic7, fp_mic7);

	PrepareWaveHead( &waveheadref, 
		1,
		MIC_RATE,
		16,
		totalSampleCountper * sizeof(unsigned short)
		);
	WriteWaveHead( &waveheadref, fp_ref);
